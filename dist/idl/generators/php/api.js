...
class PHPAPIGenerator extends generator_1.Generator {
    ...
    generateBaseRouteFile(route, eol) {
        ...
        lines.push(`		$response->headers->{"Content-Type"} = $response->headers->{"Content-Type"} ?? "${(0, schema_1.getContentTypeFromType)(route.response.payload)}";`);
    ...
    generateAutoguardFile(eol) {
        ...
        lines.push(`		global $undefined;`);
        lines.push(`		$observed = $subject === $undefined ? "undefined" : strtolower(gettype($subject));`);
        ...
        lines.push(`	}`);
        lines.push(``);
        lines.push(`	function accept_component(string $component): bool {`);
        ...
        lines.push(`			$finished = true;`);
        lines.push(`			foreach (array_slice($matchers, $i) as $matcher) {`);
        lines.push(`				if ($matcher->accept_component($component)) {`);
        ...
        lines.push(`				}`);
        lines.push(`			}`);
        lines.push(`			if ($finished) {`);
        lines.push(`				break;`);
        lines.push(`			}`);
        lines.push(`		}`);
        lines.push(`		if ($i >= count($matchers)) {`);
        lines.push(`			return false;`);
        lines.push(`		}`);
        lines.push(`		foreach (array_slice($matchers, $i) as $matcher) {`);
        lines.push(`			if (!$matcher->is_satisfied()) {`);
